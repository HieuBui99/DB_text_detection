...
from repvgg_model import RepVGGDBTextModel
...
def get_data_loaders(cfg):
    ...
    dump_dir = cfg.data[dataset_name].dump_dir
    no_imgs = cfg.data[dataset_name].no_imgs
    train_otf = cfg.data[dataset_name].train_otf
    ...
    if dataset_name == 'totaltext':
        ...
        from data_loaders import SynthTextDatasetIter
        TextDatasetIter = SynthTextDatasetIter
    elif dataset_name == 'BKAI':
        from data_loaders import ICDAR2015DatasetIter
        TextDatasetIter = ICDAR2015DatasetIter
    else:
...
def main(cfg):
    ...
    log_config = {
        "batch_size": cfg.hps.batch_size,
        "img_size": cfg.hps.img_size,
        "epochs": cfg.hps.no_epochs,
        "scheduler": cfg.lrs.mode,
        "backbone": cfg.model.backbone,
        "lr": cfg.optimizer.lr,
        "wd": cfg.optimizer.weight_decay
    }
    wandb.init(
        project="BKAI text detection",
        config=log_config
    )
    # set determinism
    ...
    use_eval = cfg.meta.use_eval
    logger.info("Use eval: {}".format(use_eval))
    ...
    logger.info("Batch size: {}".format(cfg.hps.batch_size))

    backbone_name = cfg.model.backbone
    if backbone_name == "resnet":
        dbnet = DBTextModel(mini=False).to(device)
    elif backbone_name == "mini_resnet":
        logger.info("Use mini_resnet")
        dbnet = DBTextModel(mini=True).to(device)
    elif backbone_name == "regvgg":
        dbnet = RepVGGDBTextModel().to(device)
    else:
        raise NotImplementError("Choose a correct backbone name!")
    total_params = sum(p.numel() for p in dbnet.parameters()
                       if p.requires_grad)
    ...
    if lrs_mode == 'poly':
        db_scheduler = WarmupPolyLR(db_optimizer,
                                    max_iters=cfg.hps.no_epochs * len(totaltext_train_loader),
    ...
    for epoch in range(cfg.hps.no_epochs):
        ...
        for batch_index, batch in tqdm(enumerate(totaltext_train_loader), total=len(totaltext_train_loader)):
            ...
            if global_steps % 1000 == 0:
                ...
                torch.save(
                    dbnet.state_dict(),
                    os.path.join(cfg.meta.root_dir,
                                 cfg.model.best_hmean_cp_path))

            if global_steps % cfg.hps.log_iter == 0:
                wandb.log(
                    {"train_loss": (train_loss / (batch_index + 1)).item(),
                     "global_steps": global_steps,
                     "train_total_loss": total_loss.item(),
                     "train_acc": acc,
                     "train_iou": iou_shrink_map}
                )
                logger.info(
                    "[{}-{}] - lr: {} - batch_loss: {} - total_loss: {} - loss: {} - acc: {} - iou: {}"  # noqa
                    .format(epoch + 1, global_steps, lr,
                            train_loss / (batch_index + 1), total_loss,
                            prob_threshold_loss, acc, iou_shrink_map))

        end_epoch_loss = train_loss / len(totaltext_train_loader)
        wandb.log(
                    {"train_epoch_loss": end_epoch_loss.item(), "epoch": epoch},
                )
        ...
        torch.save(dbnet.state_dict(),
                   os.path.join(cfg.meta.root_dir, cfg.model.last_cp_path + f"_{epoch}.pth"))
        wandb.log_artifact(
        os.path.join(cfg.meta.root_dir, cfg.model.last_cp_path + f"_{epoch}.pth"),
        name='epoch_checkpoint', type='weight') 
        if use_eval:
            # EVAL
            dbnet.eval()
            ...
            test_loss = 0
            raw_metrics = []
            test_visualize_index = random.choice(
                range(len(totaltext_test_loader)))
            for test_batch_index, test_batch in tqdm(
                ...
                with torch.no_grad():
                    ...
                    # tfb_writer.add_scalar('TEST/LOSS/val_loss',
                    #                       test_total_loss, global_steps)
                    ...
                    # Cal P/R/Hmean
                    batch_shape = {
                        'shape': [(cfg.hps.img_size, cfg.hps.img_size)]
                    }
            ...
            if test_loss <= best_test_loss and train_loss <= best_train_loss:
                ...
                wandb.log_artifact(
                    os.path.join(cfg.meta.root_dir, cfg.model.best_cp_path),
                    name='best_loss', type='weight'
                ) 
            if lrs_mode == 'reduce':
                db_scheduler.step(test_loss)

        if lrs_mode == 'reduce':
            db_scheduler.step(end_epoch_loss)
    ...
    torch.save(dbnet.state_dict(),
               os.path.join(cfg.meta.root_dir, cfg.model.final_cp_path))
    wandb.log_artifact(
        os.path.join(cfg.meta.root_dir, cfg.model.final_cp_path),
        name='last_check_point', type='weight') 
    # wandb.log_artifact(
    #     os.path.join(cfg.meta.root_dir, cfg.logging.logger_file),
    #     name='log', type='log')      
